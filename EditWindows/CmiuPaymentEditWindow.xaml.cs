...
{
    /// <summary>
    /// Interaction logic for CmiuPaymentEditWindow.xaml
    /// </summary>
    internal partial class CmiuPaymentEditWindow : Window, IDisposable
    {
        private readonly IHostApplicationLifetime _applicationLifetime;

        private readonly DataContext _context;
        private readonly ObservableCollection<CmiuPaymentData> observableCollection;

        private bool _justCreatedNewItem;
        private bool disposed;

        private CmiuPaymentData ClickedItem { get; set; }

        public int LinesChanged { get; private set; }

        public CmiuPaymentEditWindow(Connection connection, IHostApplicationLifetime applicationLifetime)
        {
            InitializeComponent();
            ...
            CollectionViewSource itemCollectionViewSource = (CollectionViewSource)FindResource("CmiuPaymentDataSource");
            _context.CmiuPaymentDataSet.Load();
            observableCollection = _context.CmiuPaymentDataSet.Local.ToObservableCollection();
        ...
        private void OnMouseDown(object sender, MouseButtonEventArgs e)
        {
            ...
            {
                ClickedItem = null;
                if (e.OriginalSource is TextBlock textBlock && textBlock.DataContext is CmiuPaymentData cmiuPaymentData1)
                    ClickedItem = cmiuPaymentData1;
                if (e.OriginalSource is Border border && border.DataContext is CmiuPaymentData cmiuPaymentData2)
                    ClickedItem = cmiuPaymentData2;
