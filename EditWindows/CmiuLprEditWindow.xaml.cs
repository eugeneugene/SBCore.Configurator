...
namespace SBCore.Configurator.EditWindows
{
    ...
    /// </summary>
    internal partial class CmiuLprEditWindow : Window, IDisposable
    {
        private readonly IHostApplicationLifetime _applicationLifetime;

        private readonly DataContext _context;
        private readonly ObservableCollection<CmiuLprData> observableCollection;

        private bool _justCreatedNewItem;
        private bool disposed;

        private CmiuLprData ClickedItem { get; set; }

        public int LinesChanged { get; private set; }

        public CmiuLprEditWindow(Connection connection, IHostApplicationLifetime applicationLifetime)
        {
            InitializeComponent();
            ...
            CollectionViewSource itemCollectionViewSource = (CollectionViewSource)FindResource("CmiuLprDataSource");
            _context.CmiuLprDataSet.Load();
            observableCollection = _context.CmiuLprDataSet.Local.ToObservableCollection();
        ...
        private void OnMouseDown(object sender, MouseButtonEventArgs e)
        {
            ...
            {
                ClickedItem = null;
                if (e.OriginalSource is TextBlock textBlock && textBlock.DataContext is CmiuLprData cmiuLprData1)
                    ClickedItem = cmiuLprData1;
                if (e.OriginalSource is Border border && border.DataContext is CmiuLprData cmiuLprData2)
                    ClickedItem = cmiuLprData2;
