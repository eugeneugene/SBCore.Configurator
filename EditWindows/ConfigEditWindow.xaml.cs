...
{
    internal partial class ConfigEditWindow : Window, IDisposable
    {
        private readonly IHostApplicationLifetime _applicationLifetime;

        private readonly DataContext _context;
        private readonly ObservableCollection<ConfigData> observableCollection;

        private bool _justCreatedNewItem;
        private bool disposed;

        private ConfigData ClickedItem { get; set; }

        public int LinesChanged { get; private set; }

        public ConfigEditWindow(Connection connection, IHostApplicationLifetime applicationLifetime)
        {
            InitializeComponent();
            ...
            CollectionViewSource itemCollectionViewSource = (CollectionViewSource)FindResource("ConfigDataSource");
            _context.ConfigDataSet.Load();
            observableCollection = _context.ConfigDataSet.Local.ToObservableCollection();
        ...
        private void OnMouseDown(object sender, MouseButtonEventArgs e)
        {
            ...
            {
                ClickedItem = null;
                if (e.OriginalSource is TextBlock textBlock && textBlock.DataContext is ConfigData ConfigData1)
                    ClickedItem = ConfigData1;
                if (e.OriginalSource is Border border && border.DataContext is ConfigData ConfigData2)
                    ClickedItem = ConfigData2;
