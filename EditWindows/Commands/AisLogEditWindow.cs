...
{
    internal partial class AisLogEditWindow
    {
        private void PreviewCanExecute(object sender, CanExecuteRoutedEventArgs e)
        {
            if (sender is DataGrid grid && e.Command == DataGrid.DeleteCommand)
            {
                ...
                {
                    var selectedItems = grid.SelectedItems.Cast<AisLogData>().ToArray();
        ...
        {
            if (sender is AisLogEditWindow window)
            {
                var grid = window.AisLogGrid;
                if (grid.CurrentItem is AisLogData)
        ...
        {
            observableCollection.Add(new AisLogData());
        ...
        private void RemoveRowExecuted(object sender, ExecutedRoutedEventArgs e)
        {
            if (sender is AisLogEditWindow window)
            {
                var selectedItems = new List<AisLogData> { };
                var grid = window.AisLogGrid;
                grid.CancelEdit();
                if (grid.SelectedItems.Count > 0)
                    selectedItems.AddRange(grid.SelectedItems.Cast<AisLogData>());
                else
                {
                    if (grid.CurrentItem is AisLogData item)
