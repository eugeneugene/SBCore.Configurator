...
{
    internal partial class ExemptionProvidersEditWindow
    {
        private void PreviewCanExecute(object sender, CanExecuteRoutedEventArgs e)
        {
            if (sender is DataGrid grid && e.Command == DataGrid.DeleteCommand)
            {
                ...
                {
                    var selectedItems = grid.SelectedItems.Cast<OAProviderData>().ToArray();
        ...
        {
            if (sender is ExemptionProvidersEditWindow)
            {
                e.CanExecute = true;
                e.Handled = true;
            }
        ...
        {
            if (sender is ExemptionProvidersEditWindow window)
            {
                var grid = window.ProvidersMapGrid;
                if (grid.CurrentItem is OAProviderData)
        ...
        {
            observableCollection.Add(new OAProviderData());
        ...
        private void RemoveRowExecuted(object sender, ExecutedRoutedEventArgs e)
        {
            if (sender is ExemptionProvidersEditWindow window)
            {
                var selectedItems = new List<OAProviderData> { };
                var grid = window.ProvidersMapGrid;
                grid.CancelEdit();
                if (grid.SelectedItems.Count > 0)
                    selectedItems.AddRange(grid.SelectedItems.Cast<OAProviderData>());
                else
                {
                    if (grid.CurrentItem is OAProviderData item)
