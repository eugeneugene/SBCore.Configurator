...
{
    internal partial class CmiuParkingLevelEditWindow
    {
        private void PreviewCanExecute(object sender, CanExecuteRoutedEventArgs e)
        {
            if (sender is DataGrid grid && e.Command == DataGrid.DeleteCommand)
            {
                ...
                {
                    var selectedItems = grid.SelectedItems.Cast<CmiuParkingLevelData>().ToArray();
        ...
        {
            if (sender is CmiuParkingLevelEditWindow)
            {
                e.CanExecute = true;
                e.Handled = true;
            }
        ...
        {
            if (sender is CmiuParkingLevelEditWindow window)
            {
                var grid = window.ParkingLevelMapGrid;
                if (grid.CurrentItem is CmiuParkingLevelData)
        ...
        {
            observableCollection.Add(new CmiuParkingLevelData());
        ...
        private void RemoveRowExecuted(object sender, ExecutedRoutedEventArgs e)
        {
            if (sender is CmiuParkingLevelEditWindow window)
            {
                var selectedItems = new List<CmiuParkingLevelData> { };
                var grid = window.ParkingLevelMapGrid;
                grid.CancelEdit();
                if (grid.SelectedItems.Count > 0)
                    selectedItems.AddRange(grid.SelectedItems.Cast<CmiuParkingLevelData>());
                else
                {
                    if (grid.CurrentItem is CmiuParkingLevelData cmiuParkingLevel)
                        selectedItems.Add(cmiuParkingLevel);
                }
