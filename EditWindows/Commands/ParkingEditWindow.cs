...
{
    internal partial class ParkingEditWindow
    {
        private void PreviewCanExecute(object sender, CanExecuteRoutedEventArgs e)
        {
            if (sender is DataGrid grid && e.Command == DataGrid.DeleteCommand)
            {
                ...
                {
                    var selectedItems = grid.SelectedItems.Cast<ParkingData>().ToArray();
        ...
        {
            if (sender is ParkingEditWindow)
            {
                e.CanExecute = true;
                e.Handled = true;
            }
        ...
        {
            if (sender is ParkingEditWindow window)
            {
                var grid = window.ParkingMapGrid;
                if (grid.CurrentItem is ParkingData)
        ...
        {
            observableCollection.Add(new ParkingData());
        ...
        private void RemoveRowExecuted(object sender, ExecutedRoutedEventArgs e)
        {
            if (sender is ParkingEditWindow window)
            {
                var selectedItems = new List<ParkingData> { };
                var grid = window.ParkingMapGrid;
                grid.CancelEdit();
                if (grid.SelectedItems.Count > 0)
                    selectedItems.AddRange(grid.SelectedItems.Cast<ParkingData>());
                else
                {
                    if (grid.CurrentItem is ParkingData item)
