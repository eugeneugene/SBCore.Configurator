...
{
    internal partial class CmiuLevelsEditWindow
    {
        private void PreviewCanExecute(object sender, CanExecuteRoutedEventArgs e)
        {
            if (sender is DataGrid grid && e.Command == DataGrid.DeleteCommand)
            {
                ...
                {
                    var selectedItems = grid.SelectedItems.Cast<CmiuLevelsData>().ToArray();
        ...
        {
            if (sender is CmiuLevelsEditWindow)
            {
                e.CanExecute = true;
                e.Handled = true;
            }
        ...
        {
            if (sender is CmiuLevelsEditWindow window)
            {
                var grid = window.CmiuLevelsMapGrid;
                if (grid.CurrentItem is CmiuLevelsData)
        ...
        {
            observableCollection.Add(new CmiuLevelsData());
        ...
        private void RemoveRowExecuted(object sender, ExecutedRoutedEventArgs e)
        {
            if (sender is CmiuLevelsEditWindow window)
            {
                var selectedItems = new List<CmiuLevelsData> { };
                var grid = window.CmiuLevelsMapGrid;
                grid.CancelEdit();
                if (grid.SelectedItems.Count > 0)
                    selectedItems.AddRange(grid.SelectedItems.Cast<CmiuLevelsData>());
                else
                {
                    if (grid.CurrentItem is CmiuLevelsData item)
