...
{
    internal partial class CmiuMovementEditWindow
    {
        private void PreviewCanExecute(object sender, CanExecuteRoutedEventArgs e)
        {
            if (sender is DataGrid grid && e.Command == DataGrid.DeleteCommand)
            {
                ...
                {
                    var selectedItems = grid.SelectedItems.Cast<CmiuMovementData>().ToArray();
        ...
        {
            if (sender is CmiuMovementEditWindow)
            {
                e.CanExecute = true;
                e.Handled = true;
            }
        ...
        {
            if (sender is CmiuMovementEditWindow window)
            {
                var grid = window.CmiuMovementGrid;
                if (grid.CurrentItem is CmiuMovementData)
        ...
        {
            observableCollection.Add(new CmiuMovementData());
        ...
        private void RemoveRowExecuted(object sender, ExecutedRoutedEventArgs e)
        {
            if (sender is CmiuMovementEditWindow window)
            {
                var selectedItems = new List<CmiuMovementData> { };
                var grid = window.CmiuMovementGrid;
                grid.CancelEdit();
                if (grid.SelectedItems.Count > 0)
                    selectedItems.AddRange(grid.SelectedItems.Cast<CmiuMovementData>());
                else
                {
                    if (grid.CurrentItem is CmiuMovementData item)
